...
if !MiqExpression::Field.method_defined?(:virtual_reflection?)
  class MiqExpression::Field
    ...
    def virtual_reflection?
      x = associations.present? && (model.follow_associations_with_virtual(associations) != model.follow_associations(associations))
     # byebug
      #byebug if x != (reflections != collect_reflections)
      x
...
class ReportSanityChecker
  ...
  def check_report(filename)
    ...
    if verbose
      if filename.kind_of?(MiqReport)
        ...
        name << " *** UNKNOWN ***" unless rpt.db_class
  ...
  def print_details(tbl, rpt)
    ...
    # TODO: not sure what we should high-light here
    ...
    (miq_col_names - rpt.col_order - includes_cols - rpt_cols).each do |col|
      ...
      in_sort = sort_cols.include?(col) ? "sort" : ""
      in_miq  = miq_col_names.include?(col) ? "cond" : "" # true by definition
      print_row(tbl, klass, col, in_rpt, in_inc, in_sort, in_col, in_miq)
    end
    ...
    # ---
    # a good summary, but all this data is in main table now
    # if(exp = rpt.conditions) # miq_expression
    #   # TODO: extract fields from the miq_expression and get into tbl.print_col
    #   exp_sql, exp_includes, exp_attrs = exp.to_sql(nil)
    #   sql_filter = exp_attrs[:supported_by_sql]
    #   puts "","filter: #{sql_filter ? "sql" : "ruby"}"
    # end
  end
